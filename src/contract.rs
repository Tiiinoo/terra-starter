...
use crate::msg::{CapResponse, InstantiateMsg, ExecuteMsg, QueryMsg, ScoreResponse, MigrateMsg};
...
fn query_cap(deps: Deps) -> StdResult<CapResponse> {
    let state = STORAGE.load(deps.storage)?;
    Ok(CapResponse { cap: state.cap })
}
